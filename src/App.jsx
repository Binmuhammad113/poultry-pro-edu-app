...
import './App.css'
import { addFarmEntity, deleteFarmEntity, getFarmSummary, updateFarmEntity, withFarmRecord } from './domain/farm'
...
function ManagementView({ activeNav, onAddRecord, onEditRecord, onDeleteRecord, farm, query, highlightMatches }) {
  return <LiveManagementView activeNav={activeNav} onAddRecord={onAddRecord} onEditRecord={onEditRecord} onDeleteRecord={onDeleteRecord} farm={farm} query={query} highlightMatches={highlightMatches} />
}

function LiveManagementView({ activeNav, onAddRecord, onEditRecord, onDeleteRecord, farm, query, highlightMatches }) {
  const summary = getFarmSummary(farm)
  const flocks = farm.flocks.filter((flock) => !query || [flock.name, flock.type, flock.age, flock.health].some((field) => String(field).toLowerCase().includes(query)))
  const inventory = farm.inventory.filter((item) => !query || [item.item, item.category, item.quantity, item.level].some((field) => String(field).toLowerCase().includes(query)))
  const finances = farm.finances.filter((transaction) => !query || [transaction.description, transaction.type, transaction.amount].some((field) => String(field).toLowerCase().includes(query)))
  const stats = activeNav === 'Flocks'
    ? [['Active houses', farm.flocks.length], ['Total birds', summary.totalBirds.toLocaleString()], ['Average mortality', '0%'], ['Next vaccination', farm.flocks.length ? 'Not scheduled' : 'No flocks yet']]
    : activeNav === 'Inventory'
      ? [['Tracked items', farm.inventory.length], ['Feed on hand', `${summary.feedStockTons.toFixed(1)}t`], ['Low stock items', summary.lowStockItems], ['Stock value', '₦0']]
      : [['Revenue this month', formatManagementCurrency(summary.totalRevenue)], ['Operating costs', formatManagementCurrency(summary.totalExpenses)], ['Net profit', formatManagementCurrency(summary.netProfit)], ['Profit margin', summary.totalRevenue ? `${Math.round((summary.netProfit / summary.totalRevenue) * 100)}%` : '0%']]

  if (activeNav === 'Reports') return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Farm intelligence</p><h2>Reports</h2><p>Reports are calculated from your current farm records.</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>＋</span> Add record</button></div><div className="panel report-preview live-report"><div className="panel-heading"><div><h2>Farm data status</h2><p>No demo values are used</p></div><span className="health-pill good">● {farm.flocks.length || farm.inventory.length || farm.finances.length ? 'Data available' : 'No data yet'}</span></div><div className="empty-state">{farm.flocks.length || farm.inventory.length || farm.finances.length ? 'Your reports will update as you record production, stock, and transactions.' : 'Add flocks, inventory, or transactions to generate reports.'}</div></div></section>

  const rows = activeNav === 'Flocks' ? flocks : activeNav === 'Inventory' ? inventory : finances
  const title = activeNav === 'Flocks' ? 'Flocks' : activeNav === 'Inventory' ? 'Inventory' : 'Finances'
  const description = activeNav === 'Flocks' ? 'Monitor every house, flock age, and health status from one place.' : activeNav === 'Inventory' ? 'Know what is available before the next feeding, treatment, or collection.' : 'Track income, operating costs, and the health of your farm business.'
  return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Farm records</p><h2>{title}</h2><p>{description}</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>＋</span> Add {activeNav === 'Flocks' ? 'flock' : activeNav === 'Inventory' ? 'stock' : 'transaction'}</button></div><div className="management-stats">{stats.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><article className="panel management-table"><div className="panel-heading"><div><h2>{rows.length ? `${rows.length} recorded ${title.toLowerCase()}` : `No ${title.toLowerCase()} recorded`}</h2><p>Only data entered by your farm appears here.</p></div></div>{rows.length ? <div className="table-wrap"><table><thead><tr>{activeNav === 'Flocks' ? <><th>HOUSE</th><th>TYPE</th><th>AGE</th><th>BIRDS</th><th>HEALTH</th><th /></> : activeNav === 'Inventory' ? <><th>ITEM</th><th>CATEGORY</th><th>QUANTITY</th><th>STATUS</th><th /></> : <><th>DESCRIPTION</th><th>TYPE</th><th>AMOUNT</th><th /></>}</tr></thead><tbody>{rows.map((record) => <tr key={record.id}>{activeNav === 'Flocks' ? <><td>{highlightMatches(record.name)}</td><td>{highlightMatches(record.type)}</td><td>{highlightMatches(record.age)}</td><td>{Number(record.birds || 0).toLocaleString()}</td><td>{highlightMatches(record.health)}</td></> : activeNav === 'Inventory' ? <><td>{highlightMatches(record.item)}</td><td>{highlightMatches(record.category)}</td><td>{highlightMatches(record.quantity)}</td><td>{highlightMatches(record.level)}</td></> : <><td>{highlightMatches(record.description)}</td><td>{highlightMatches(record.type)}</td><td>{highlightMatches(record.amount)}</td></>}<td><button type="button" className="row-menu" onClick={() => onEditRecord(activeNav === 'Flocks' ? 'flock' : activeNav === 'Inventory' ? 'inventory' : 'finance', record)}>Edit</button><button type="button" className="row-delete" onClick={() => onDeleteRecord(activeNav === 'Flocks' ? 'flock' : activeNav === 'Inventory' ? 'inventory' : 'finance', record)}>×</button></td></tr>)}</tbody></table></div> : <div className="empty-state">Start by using the Add button above.</div>}</article></section>
}
...
function _LegacyManagementView({ activeNav, onAddRecord, onEditRecord, onDeleteRecord, farm, query, highlightMatches }) {
  if (activeNav === 'Flocks') {
    return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Production management</p><h2>Flocks</h2><p>Monitor every house, flock age, and health status from one place.</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>＋</span> Add flock</button></div><div className="management-stats"><div><span>Active houses</span><strong>3</strong></div><div><span>Total birds</span><strong>5,650</strong></div><div><span>Average mortality</span><strong>2.1%</strong></div><div><span>Next vaccination</span><strong>Sep 12</strong></div></div><article className="panel management-table"><div className="panel-heading"><div><h2>All active flocks</h2><p>Updated from the latest health checks</p></div><button type="button" className="select-button">All flock types <span>⌄</span></button></div><div className="table-wrap"><table><thead><tr><th>HOUSE</th><th>TYPE</th><th>AGE</th><th>BIRDS</th><th>HEALTH</th><th>LAST CHECK</th><th /></tr></thead><tbody>{filteredFlocks.length ? filteredFlocks.map((flock) => <tr key={flock.name}><td><span className="house-dot" />{highlightMatches(flock.name)}</td><td>{highlightMatches(flock.type)}</td><td>{highlightMatches(flock.age)}</td><td><strong>{flock.birds}</strong></td><td><span className={`health-pill ${flock.tone}`}>● {highlightMatches(flock.health)}</span></td><td>Today, 08:30</td><td><button type="button" className="row-menu" aria-label={`Edit ${flock.name}`} onClick={() => onEditRecord('flock', flock)}>•••</button><button type="button" className="row-delete" aria-label={`Delete ${flock.name}`} onClick={() => onDeleteRecord('flock', flock)}>×</button></td></tr>) : <tr><td colSpan="7"><div className="empty-state table-empty">No flocks match your search.</div></td></tr>}</tbody></table></div></article></section>
  }

  if (activeNav === 'Inventory') {
    return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Stock control</p><h2>Inventory</h2><p>Know what is available before the next feeding, treatment, or collection.</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>＋</span> Add stock</button></div><div className="management-stats"><div><span>Tracked items</span><strong>24</strong></div><div><span>Feed on hand</span><strong>2.4t</strong></div><div><span>Low stock items</span><strong className="attention-text">2</strong></div><div><span>Stock value</span><strong>₦1.18m</strong></div></div><article className="panel management-table"><div className="panel-heading"><div><h2>Current inventory</h2><p>Feed, medication, and farm supplies</p></div><button type="button" className="select-button">All categories <span>⌄</span></button></div><div className="table-wrap"><table><thead><tr><th>ITEM</th><th>CATEGORY</th><th>QUANTITY</th><th>STATUS</th><th>LAST UPDATED</th><th /></tr></thead><tbody>{filteredInventory.length ? filteredInventory.map((item) => <tr key={item.item}><td><span className="inventory-dot" />{highlightMatches(item.item)}</td><td>{highlightMatches(item.category)}</td><td><strong>{highlightMatches(item.quantity)}</strong></td><td><span className={`health-pill ${item.tone}`}>● {highlightMatches(item.level)}</span></td><td>{highlightMatches(item.updated)}</td><td><button type="button" className="row-menu" aria-label={`Edit ${item.item}`} onClick={() => onEditRecord('inventory', item)}>•••</button><button type="button" className="row-delete" aria-label={`Delete ${item.item}`} onClick={() => onDeleteRecord('inventory', item)}>×</button></td></tr>) : <tr><td colSpan="6"><div className="empty-state table-empty">No inventory item matches your search.</div></td></tr>}</tbody></table></div></article></section>
  }

  if (activeNav === 'Finances') {
    return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Farm accounting</p><h2>Finances</h2><p>Track income, operating costs, and the health of your farm business.</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>＋</span> Add transaction</button></div><div className="management-stats"><div><span>Revenue this month</span><strong>₦2.84m</strong></div><div><span>Operating costs</span><strong>₦1.16m</strong></div><div><span>Net profit</span><strong className="income-text">₦1.68m</strong></div><div><span>Profit margin</span><strong>59.2%</strong></div></div><article className="panel management-table"><div className="panel-heading"><div><h2>Recent transactions</h2><p>Income and expenses recorded for Ogun Green Farm</p></div><button type="button" className="select-button">This month <span>⌄</span></button></div><div className="table-wrap"><table><thead><tr><th>DESCRIPTION</th><th>TYPE</th><th>AMOUNT</th><th>DATE</th><th /></tr></thead><tbody>{filteredFinance.length ? filteredFinance.map((transaction) => <tr key={transaction.description}><td><span className="transaction-dot" />{highlightMatches(transaction.description)}</td><td>{highlightMatches(transaction.type)}</td><td><strong className={transaction.tone === 'income' ? 'income-text' : 'expense-text'}>{highlightMatches(transaction.amount)}</strong></td><td>{highlightMatches(transaction.date)}</td><td><button type="button" className="row-menu" aria-label={`Edit ${transaction.description}`} onClick={() => onEditRecord('finance', transaction)}>•••</button><button type="button" className="row-delete" aria-label={`Delete ${transaction.description}`} onClick={() => onDeleteRecord('finance', transaction)}>×</button></td></tr>) : <tr><td colSpan="5"><div className="empty-state table-empty">No financial records match your search.</div></td></tr>}</tbody></table></div></article></section>
  }

  return <section className="management-view"><div className="management-header"><div><p className="eyebrow">Farm intelligence</p><h2>Reports</h2><p>Turn your farm records into decisions you can act on.</p></div><button type="button" className="primary-button" onClick={onAddRecord}><span>↓</span> Export report</button></div>{query ? <div className="empty-state">No report cards match your search. Try a broader keyword.</div> : <><div className="report-grid"><article className="panel report-card"><span className="report-icon green-bg">▤</span><h3>Flock health report</h3><p>Mortality, vaccination, and health observations across all houses.</p><button type="button" className="text-button">Open report <span>→</span></button></article><article className="panel report-card"><span className="report-icon amber-bg">₦</span><h3>Profitability report</h3><p>Compare sales, feed costs, and margins by flock and production cycle.</p><button type="button" className="text-button">Open report <span>→</span></button></article><article className="panel report-card"><span className="report-icon blue-bg">◌</span><h3>Production report</h3><p>Review eggs, bird growth, feed conversion, and daily farm output.</p><button type="button" className="text-button">Open report <span>→</span></button></article></div><article className="panel report-preview"><div className="panel-heading"><div><h2>Monthly farm health</h2><p>Calculated from current farm records</p></div><span className="health-pill good">● {farm.flocks.length ? 'Collecting data' : 'No data yet'}</span></div>{farm.flocks.length || farm.finances.length || farm.inventory.length ? <div className="report-bars"><div><span>Flock health</span><i><b style={{ width: `${farm.flocks.length ? 100 : 0}%` }} /></i><strong>{farm.flocks.length ? '100%' : '0%'}</strong></div><div><span>Feed efficiency</span><i><b style={{ width: `${summary.feedStockTons ? 100 : 0}%` }} /></i><strong>{summary.feedStockTons ? '100%' : '0%'}</strong></div><div><span>Egg production</span><i><b style={{ width: `${summary.eggsCollected ? 100 : 0}%` }} /></i><strong>{summary.eggsCollected ? '100%' : '0%'}</strong></div><div><span>Profit target</span><i><b style={{ width: `${summary.totalRevenue ? 100 : 0}%` }} /></i><strong>{summary.totalRevenue ? '100%' : '0%'}</strong></div></div> : <div className="empty-state">Add flocks, inventory, or transactions to generate reports.</div>}</article></>}</section>
}

function EditRecordModal({ record, entityType, onClose, onSave }) {
  const submit = (event) => {
    event.preventDefault()
    onSave(Object.fromEntries(new FormData(event.currentTarget).entries()))
  }

  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="record-modal" onSubmit={submit}><div className="modal-header"><div><p className="eyebrow">Farm records</p><h2>Edit {entityType === 'flock' ? 'flock' : entityType === 'inventory' ? 'stock' : 'transaction'}</h2></div><button type="button" className="close-button" onClick={onClose} aria-label="Close">×</button></div>{entityType === 'flock' && <><label>House name<input name="name" defaultValue={record.name} required /></label><label>Bird type<select name="type" defaultValue={record.type}><option>Broilers</option><option>Layers</option><option>Breeders</option></select></label><label>Bird count<input name="birds" type="number" min="1" defaultValue={record.birds} required /></label><label>Flock age<input name="age" defaultValue={record.age} required /></label></>}{entityType === 'inventory' && <><label>Item name<input name="item" defaultValue={record.item} required /></label><label>Category<select name="category" defaultValue={record.category}><option>Feed</option><option>Medication</option><option>Supplies</option></select></label><label>Quantity<input name="quantity" defaultValue={record.quantity} required /></label></>}{entityType === 'finance' && <><label>Description<input name="description" defaultValue={record.description} required /></label><label>Type<select name="type" defaultValue={record.type}><option>Income</option><option>Expense</option></select></label><label>Amount<input name="amount" type="number" min="0" defaultValue={String(record.amount).replace(/[^0-9]/g, '')} required /></label></>}<button type="submit" className="primary-button">Save changes</button></form></div>
}
...
function App() {
  ...
  const openEditRecord = (entityType, record) => {
    setRecordContext(entityType)
    setEditingRecord(record)
    setShowRecordForm(false)
  }

  const deleteRecord = (entityType, record) => {
    const label = record.name || record.item || record.description
    if (!window.confirm(`Delete ${label}? This cannot be undone.`)) return
    setFarm((current) => deleteFarmEntity(current, entityType, record.id))
  }

  const saveEditedRecord = (values) => {
    setFarm((current) => updateFarmEntity(current, recordContext, editingRecord.id, values))
    setEditingRecord(null)
  }
  ...
  const handleRecordSubmit = (event) => {
    ...
    setFarm((current) => recordContext === 'record'
      ...
      : editingRecord ? updateFarmEntity(current, recordContext, editingRecord.id, values) : addFarmEntity(current, recordContext, values))
    setRecordSaved(true)
    setTimeout(() => {
      setShowRecordForm(false)
      setEditingRecord(null)
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          <section className="stats-grid" aria-label="Farm summary">
            <article className="stat-card accent-green"><div className="stat-icon">◌</div><span className="stat-label">Total birds</span><strong>{summary.totalBirds.toLocaleString()}</strong><small className={summary.totalBirds ? 'positive' : 'muted-stat'}>{summary.totalBirds ? 'Live farm total' : 'Add a flock to begin'}</small><div className="sparkline green-line"><i /><i /><i /><i /><i /><i /><i /></div></article>
            <article className="stat-card accent-amber"><div className="stat-icon">▦</div><span className="stat-label">Eggs collected</span><strong>{summary.eggsCollected.toLocaleString()} <small>/ {summary.eggTarget.toLocaleString()}</small></strong><small className={summary.eggTarget ? 'positive' : 'muted-stat'}>{summary.eggTarget ? 'From recorded production' : 'Add production records'}</small><div className="progress-track"><span style={{ width: `${summary.eggTarget ? Math.min(100, (summary.eggsCollected / summary.eggTarget) * 100) : 0}%` }} /></div></article>
            <article className="stat-card accent-blue"><div className="stat-icon">◒</div><span className="stat-label">Feed stock</span><strong>{summary.feedStockTons.toFixed(1)} <small>tons</small></strong><small className={summary.feedStockTons ? 'warning' : 'muted-stat'}>{summary.feedStockTons ? `● ${summary.feedDaysRemaining} days remaining` : 'Add feed inventory'}</small><div className="progress-track blue-track"><span style={{ width: `${Math.min(100, (summary.feedStockTons / 4.5) * 100)}%` }} /></div></article>
            <article className="stat-card accent-coral"><div className="stat-icon">₦</div><span className="stat-label">This month’s revenue</span><strong>{formatCompactCurrency(summary.totalRevenue)}</strong><small className={summary.totalRevenue ? 'positive' : 'muted-stat'}>{summary.totalRevenue ? 'From recorded income' : 'Add a transaction'}</small><div className="sparkline coral-line"><i /><i /><i /><i /><i /><i /><i /></div></article>
          </section>

          <section className="dashboard-grid">
            <article className="panel performance-panel"><div className="panel-heading"><div><h2>Flock performance</h2><p>Bird count and mortality across your houses</p></div><button type="button" className="select-button">Last 30 days <span>⌄</span></button></div>{summary.totalBirds ? <div className="chart-placeholder"><strong>{summary.totalBirds.toLocaleString()} live birds</strong><span>Performance history will appear as records accumulate.</span></div> : <div className="empty-state chart-placeholder"><strong>No flock performance yet</strong><span>Add a flock and begin recording health checks.</span></div>}</article>
          ...
          </> : <ManagementView activeNav={activeNav} onAddRecord={openRecordForm} onEditRecord={openEditRecord} onDeleteRecord={deleteRecord} farm={farm} query={query} highlightMatches={highlightMatches} />}
        </div>
      </main>
      ...
      {editingRecord && <EditRecordModal record={editingRecord} entityType={recordContext} onClose={() => setEditingRecord(null)} onSave={saveEditedRecord} />}
